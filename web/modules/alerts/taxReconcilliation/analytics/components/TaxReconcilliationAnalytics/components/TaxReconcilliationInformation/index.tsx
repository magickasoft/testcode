...
const TaxReconcilliationInformation = React.memo((properties: Properties) => {
  ...
  const renderGeneral = React.useCallback(
    () => (
      <>
        <Delimiter />
        <InfoSet direction={InfoSet.DIRECTION_ROW} legend="Tax Reconciliation Detail">
          ...
          <div className={styles.column}>
            ...
            <Info label="Month">
              {value?.start_date && <DateTime utc={value?.start_date} timeFormat="" dateFormat="MMM-YYYY" />}
            </Info>
          </div>
        </InfoSet>
      </>
  ...
  const renderActions = React.useCallback(
    () => (
      <Link button to={`${MAIN_PATH}/taxReconcilliation/edit/${value?.id}`} face={Link.FACE_SECONDARY}>
        Edit
      </Link>
    ),
    [value]
...
export { TaxReconcilliationInformation };
